...
public class JacobiDGFEM extends DGFEM {
    ...
    public JacobiDGFEM(int index, double[][] vertices, double[][] Uinit, double[] wallDistance, double[][] externalField, int[] TT, int[] TP, int[] TEale, int[] TEshift, double[][] shift, FaceCurvature fCurv, double[][] blendFun, double[] initW,
                            ...
                            for (int i = 0; i < nBasis; i++) {
                                for (int j = 0; j < nBasis; j++) {
                                    if (eqn.isConvective()) {
                                        ...
                                        if (m == q) {
                                            ADiag[nBasis * m + i][nBasis * q + j] += (eps + dampConst) * Jac * weight * dBase[i][d] * dBase[j][d];
                                        }
                                    }
                                    if (eqn.isDiffusive()) {
                        ...
                        for (int i = 0; i < nBasis; i++) {
                            for (int j = 0; j < nBasis; j++) {
                                if (eqn.isConvective()) {
                                    ...
                                    if (m == q) {
                                        ADiag[nBasis * m + i][nBasis * q + j] -= 0.5 * (0.5 * (eps + elems[TT[k]].eps) + dampConst) * Jac * weight * dBazeSumL[i] * baseLeft[j];
                                    }
                                }
                                if (eqn.isDiffusive()) {
                        ...
                        for (int i = 0; i < nRBasis; i++) {
                            for (int j = 0; j < nBasis; j++) {
                                if (eqn.isConvective()) {
                                    ...
                                    if (m == q) {
                                        Sous.A[nRBasis * m + i][nBasis * q + j] -= 0.5 * (0.5 * (eps + elems[TT[k]].eps) + dampConst) * Jac * weight * dBazeSumR[i] * baseLeft[j];
                                    }
                                }
                                if (eqn.isDiffusive()) {
                    ...
                    for (int d = 0; d < dim; d++) {
                        for (int m = 0; m < nEqs; m++) {
                            ...
                            for (int q = 0; q < nEqs; q++) {
                                ...
                                for (int r = 0; r < dim; r++) {
                                    dWch[nEqs * r + q] = (dWLh[nEqs * r + q] + dWR[nEqs * r + q]) / 2 + beta0 * (WR[q] - WL[q]) / dL * n[k][p][r];
                                }
                            }
