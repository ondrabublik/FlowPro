...
data = NACA(np,0);
xs1 = linspace(3,0,ns+1)';
xs2 = linspace(0,3,ns+1)';
...
for i = 1:n1-1
    for j = 1:n2-1
        ...
        s = s + 2;
        Q(sq,:) = [ind(i,j),ind(i+1,j),ind(i+1,j+1),ind(i,j+1)];
        sq = sq + 1;
    end
end

% unifikace
[P,I,J] = uniquetol(P+1,1e-14,'rows');
P = P-1;
TP = [J(TP(:,1)),J(TP(:,2)),J(TP(:,3))];
Q = [J(Q(:,1)),J(Q(:,2)),J(Q(:,3)),J(Q(:,4))];
